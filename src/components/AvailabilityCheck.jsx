...
const AvailabilityCheck = () => {
    ...
    return (
        <div className="card" style={{ maxWidth: '600px', margin: '0 auto' }}>
            ...
            {result && (
                <div style={{ padding: '1.5rem', background: result.Available ? '#dcfce7' : '#fee2e2', borderRadius: '12px', textAlign: 'center' }}>
                    <h3 style={{ margin: '0 0 0.5rem 0', color: result.Available ? 'var(--success)' : 'var(--danger)' }}>
                        {result.Available ? 'Available' : 'Not Available'}
                    </h3>
                    <p style={{ margin: 0, color: 'var(--text-secondary)' }}>
                        {result.Message || (result.Available ? 'The professional is free at this time.' : 'The professional has conflicts.')}
                    </p>
                </div>
            )}
        </div>
